...
out_dir <- Sys.getenv("OUT_DIR",
  "/home/lshpk18/OzanGundogdu_SOUK011275/Analysis/Anton_downstream_mapping/Sense_check")
...
progress <- file.path(out_dir, "logs", "deseq2_sense_check.log")
logmsg <- function(...) {
...
logmsg("Sense-check: technical replicate comparison for timepoint =", tp_use)
...
# ---- 2. Select the 8 samples for the chosen timepoint --------------------
#
# Naming convention:
#   infected timepoints:  15m_1_Run_1_S4, 30m_2_Run_2_S35 ...
#   0h controls:          B1_H1_Run_1_S3, B2_H2_Run_2_S33 ...
#
# For tp_use == "0h" select B*_H*_Run_* columns.
# For other timepoints select <tp>_*_Run_* columns.
...
if (length(keep) == 0)
  stop("No samples found for timepoint '", tp_use, "'. Available: ", paste(all_samples, collapse=", "))
...
p_pca <- ggplot(pcdf, aes(PC1, PC2, colour=run, label=sample_id)) +
  ...
  theme_bw() +
  labs(title=paste0("Bacteria sense-check PCA — ", tp_use, " (Run1 vs Run2)"),
       x=paste0("PC1 (", var[1], "%)"), y=paste0("PC2 (", var[2], "%)"),
       colour="Run")

# ggrepel optional — fall back if not installed
p_pca <- tryCatch(p_pca, error=function(e) {
  ggplot(pcdf, aes(PC1, PC2, colour=run)) +
    geom_point(size=4) +
    scale_colour_manual(values=c(Run1="#E41A1C", Run2="#377EB8")) +
    theme_bw() +
    labs(title=paste0("Bacteria sense-check PCA — ", tp_use, " (Run1 vs Run2)"),
...
pdf(file.path(out_dir, "figures", paste0(tp_use, "_run_sample_distances.pdf")), width=7, height=6)
pheatmap(as.matrix(dist(t(mat_vst))),
         annotation_col = as.data.frame(meta_vst[, .(run)], row.names=meta_vst$sample_id),
         annotation_colors = list(run=c(Run1="#E41A1C", Run2="#377EB8")),
         main=paste0("Bacteria sample distances — ", tp_use, " (Run1 vs Run2)"))
dev.off()
...
p_vol <- ggplot(tab, aes(log2FoldChange, -log10(pmax(padj, 1e-300)), colour=status)) +
  ...
  theme_bw() +
  labs(title=paste0("Technical replicate sense-check: ", tp_use, "  Run1 vs Run2"),
       subtitle="Expect minimal DE if runs are technically consistent",
       x="log2 fold change (Run1 / Run2)",
       y="-log10 adjusted p-value",
       colour=NULL)
save_plot(p_vol, file.path(out_dir, "figures", paste0(tp_use, "_Run1_vs_Run2_volcano")), 7, 5)
logmsg("Volcano saved:", paste0(tp_use, "_Run1_vs_Run2_volcano"))

logmsg("Sense-check complete. Results in:", out_dir)
