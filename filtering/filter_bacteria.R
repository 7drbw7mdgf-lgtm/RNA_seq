...
out_dir <- Sys.getenv("OUT_DIR",
  "/home/lshpk18/OzanGundogdu_SOUK011275/Analysis/filtering/bacteria")
...
log_file <- file.path(out_dir, "filter_bacteria.log")
logmsg <- function(...) {
...
logmsg("=== Bacteria featureCounts filtering ===")
...
# Strip full path and .bacteria.merged.bam suffix -> sample key e.g. 15m_1_Run_1_S4
setnames(counts,
  setdiff(names(counts), "gene_id"),
  sub("\\.bacteria\\.merged\\.bam$", "",
...
# ---- 2. Build metadata ----
# Run_1 and Run_2 are kept as separate observations (8 per timepoint:
# 4 biological replicates × 2 runs). Run is added as a covariate for
# downstream DESeq2 modelling (design: ~ run + timepoint).
...
# ---- 6. Per-sample total counts summary ----
sample_totals <- colSums(mat_filt)
logmsg("Per-sample total counts (filtered matrix):")
logmsg(paste(names(sample_totals), sample_totals, sep = "=", collapse = "  "))

stats <- data.table(
  step = c("raw_genes", "low_count_removed", "genes_retained"),
  n    = c(n_genes_raw, n_removed, nrow(mat_filt))
)
fwrite(stats, file.path(out_dir, "bacteria_filter_stats.tsv"), sep = "\t")

# ---- 7. Write outputs ----
out_counts <- as.data.table(mat_filt, keep.rownames = "gene_id")
fwrite(out_counts, file.path(out_dir, "bacteria_filtered_counts.tsv"), sep = "\t")
logmsg("Filtered count matrix written:",
       file.path(out_dir, "bacteria_filtered_counts.tsv"))

fwrite(meta, file.path(out_dir, "bacteria_metadata.tsv"), sep = "\t")
logmsg("Metadata written:", file.path(out_dir, "bacteria_metadata.tsv"))
...
lib_dt[, sample_id := factor(sample_id, levels = sample_id[order(timepoint, replicate, run)])]
...
p_lib <- ggplot(lib_dt, aes(sample_id, total / 1e6, fill = timepoint, alpha = run)) +
  ...
  labs(title = "Bacteria: per-sample library size (filtered counts)",
       x = NULL, y = "Total mapped reads (millions)", fill = "Timepoint", alpha = "Run")

ggsave(file.path(out_dir, "figures", "bacteria_library_sizes.pdf"), p_lib, width = 10, height = 5)
ggsave(file.path(out_dir, "figures", "bacteria_library_sizes.png"), p_lib, width = 10, height = 5, dpi = 300)
...
p_dist <- ggplot(dist_dt, aes(log10_sum, fill = stage, colour = stage)) +
  ...
  theme_bw() +
  labs(title = "Bacteria: gene count-sum distribution",
       x = "log10(row sum across all samples)", y = "Density",
       fill = NULL, colour = NULL)

ggsave(file.path(out_dir, "figures", "bacteria_count_distribution.pdf"), p_dist, width = 7, height = 5)
ggsave(file.path(out_dir, "figures", "bacteria_count_distribution.png"), p_dist, width = 7, height = 5, dpi = 300)
...
p_run <- ggplot(lib_dt, aes(run, total / 1e6, fill = run)) +
  ...
  theme_bw() +
  labs(title = "Bacteria: library size by sequencing run",
       x = NULL, y = "Total mapped reads (millions)", fill = NULL)

ggsave(file.path(out_dir, "figures", "bacteria_library_by_run.pdf"), p_run, width = 5, height = 5)
ggsave(file.path(out_dir, "figures", "bacteria_library_by_run.png"), p_run, width = 5, height = 5, dpi = 300)

logmsg("Figures saved to", file.path(out_dir, "figures"))
logmsg("=== Bacteria filtering complete ===")
