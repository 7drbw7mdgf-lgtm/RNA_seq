...
# Merge host BAM files from different lanes of the same sample+run,
...
#
# e.g.  15m_1_Run_1_S4_L001.host.Aligned.sortedByCoord.out.bam  ┐
#       15m_1_Run_1_S4_L007.host.Aligned.sortedByCoord.out.bam  ┘ -> 15m_1_Run_1_S4.host.merged.bam
#
# Result: 96 merged+indexed BAMs (48 samples x 2 runs).
#
# Usage:
#   bash index_host_bams.sh [--bam-dir PATH] [--out-dir PATH] \
#                           [--threads N] [--parallel N] [--force]

BAM_DIR="${BAM_DIR:-/home/lshpk18/OzanGundogdu_SOUK011275/Analysis/Alignment/bam/host}"
OUT_DIR="${OUT_DIR:-/home/lshpk18/OzanGundogdu_SOUK011275/Analysis/Anton_downstream_mapping/Bam_indexing/Host/merged}"
THREADS="${THREADS:-2}"
PARALLEL="${PARALLEL:-8}"
FORCE="${FORCE:-false}"
LOG_DIR="${LOG_DIR:-/home/lshpk18/OzanGundogdu_SOUK011275/Analysis/Anton_downstream_mapping/Bam_indexing/Host/logs}"
...
LOG_FILE="$LOG_DIR/merge_index_host_$(date +%Y%m%d_%H%M%S).log"

log() { echo "[$(date '+%Y-%m-%d %H:%M:%S')] $*" | tee -a "$LOG_FILE"; }

log "============================================"
log "Host BAM Lane Merge + Index"
log "============================================"
...
while IFS= read -r bam; do
  name=$(basename "$bam")
  group_key="${name%%_L0*.host*}"
  group_bams["$group_key"]+=" $bam"
done < <(find "$BAM_DIR" -maxdepth 1 -name "*.host.Aligned.sortedByCoord.out.bam" | sort)

n_groups="${#group_bams[@]}"
[[ "$n_groups" -gt 0 ]] || { log "ERROR: no host BAM files found in $BAM_DIR"; exit 1; }
...
merge_and_index() {
  local key="$1"
  local bams="$2"
  local out_bam="$OUT_DIR/${key}.host.merged.bam"
...
n_skip=0
for key in $(echo "${!group_bams[@]}" | tr ' ' '\n' | sort); do
  out_bam="$OUT_DIR/${key}.host.merged.bam"
  if [[ "$FORCE" == "false" && -f "$out_bam" && -f "${out_bam}.bai" ]]; then
    (( n_skip++ )) || true
  else
